...
> 基于 AI 辅助的智能 PID 物联网数据平台 — **云端管理平台**

---

## 📋 项目概述

AIPID 温控系统是一个面向工业 PID 温控场景的云端数据管理平台，实现**边缘侧（树莓派/PLC）与云端之间的双向数据交互**：

- **上行 ↑**：边缘侧设备实时上传 PID 运行特征帧（KP、TI、TD、IAE 等指标）到云端存储
- **下行 ↓**：云端可下发 PID 参数调整指令（含风扇功率设定）给边缘侧设备执行
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/issue_command` | 云端下发 PID 调整指令（含风扇功率设定） |
...
```
> `fan_power` 为可选字段，不传或传 `null` 表示沿用当前风扇功率。
...
| `downlink_commands` | 下行指令表 | action_batch_id(唯一), delta_kp/ti/td, confidence, applied, **fan_power** |
